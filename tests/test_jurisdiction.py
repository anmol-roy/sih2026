"""
Phase 6 — Jurisdiction Router tests  (keyword fallback — no LLM/network)
...
def _run_cases(cases, expected, label):
    passed = failed = 0
    print(f"\n── {label} ──────────────────────────────────────────────────")
    for query in cases:
...
def test_india_section3p():
    assert _keyword_classify("What does Section 3(p) of the Patents Act say?").jurisdiction == Jurisdiction.INDIA

def test_india_ayush():
    assert _keyword_classify("What does AYUSH say about traditional formulations?").jurisdiction == Jurisdiction.INDIA

def test_india_tkdl():
    assert _keyword_classify("What is the TKDL and how is it used by the Indian Patent Office?").jurisdiction == Jurisdiction.INDIA

def test_intl_pct():
    assert _keyword_classify("What is the PCT and how does it work?").jurisdiction == Jurisdiction.INTERNATIONAL

def test_intl_trips():
    assert _keyword_classify("Explain the TRIPS Agreement.").jurisdiction == Jurisdiction.INTERNATIONAL

def test_intl_wipo():
    assert _keyword_classify("How do I file an international patent application with WIPO?").jurisdiction == Jurisdiction.INTERNATIONAL

def test_both_compare_pct():
    assert _keyword_classify("Compare Indian patent law with the PCT.").jurisdiction == Jurisdiction.BOTH

def test_both_trips_india():
    assert _keyword_classify("How does India's Patents Act relate to TRIPS obligations?").jurisdiction == Jurisdiction.BOTH

def test_override_india():
    r = JurisdictionRouter(llm=None).classify("What is patentability?", override="india")
    assert r.jurisdiction == Jurisdiction.INDIA and r.confidence == 1.0

def test_override_international():
    r = JurisdictionRouter(llm=None).classify("What is patentability?", override="international")
    assert r.jurisdiction == Jurisdiction.INTERNATIONAL and r.confidence == 1.0

def test_override_both():
    r = JurisdictionRouter(llm=None).classify("How does IP law work?", override="both")
    assert r.jurisdiction == Jurisdiction.BOTH and r.confidence == 1.0

def test_default_to_india():
